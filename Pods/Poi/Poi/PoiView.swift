...
public class PoiView: UIView {
    ...
    private func setUp() {
        ...
        for iterator in (0..<countOfCards) {
            contentViews.append(createCard(index: iterator))
        }
        for iterator in (0..<countOfCards).reversed() {
            contentViews[iterator].frame = CGRect(x: 0, y: 0, width: self.frame.width, height: self.frame.height)
            self.addSubview(contentViews[iterator])
        }
